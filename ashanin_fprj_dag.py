...
dag = DAG(
    USERNAME + '_frpj_dwh_etl',
    default_args = default_args,
    description = 'frpj DWH ETL tasks',
...
del_ods = PostgresOperator(
    ...
    dag=dag,
    sql="""
        DELETE FROM rtk_de.ashanin_fprj_ods_payment WHERE EXTRACT(YEAR FROM pay_date::DATE) = {{ execution_date.year }};
        """
)
...
ins_ods = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_ods_payment (SELECT * FROM rtk_de.ashanin_fprj_stg_payment WHERE EXTRACT(YEAR FROM pay_date::DATE) = {{ execution_date.year }});
        """
)
...
ins_hub_user = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_hub_user (user_pk, user_key, load_date, record_source)
    (SELECT user_pk, user_key, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_hub_user_etl);
    """
)
...
ins_hub_account = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_hub_account (ACCOUNT_PK, account_key, load_date, record_source)
    (SELECT ACCOUNT_PK, account_key, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_hub_account_etl);
    """
)
...
ins_hub_payment = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_hub_payment (pay_pk, pay_doc_type_id, pay_doc_num, load_date, record_source)
    (SELECT pay_pk, pay_doc_type_id, pay_doc_num, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_hub_payment_etl);
    """
)
...
ins_hub_billing_period = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_hub_billing_period (billing_period_pk, billing_period_key, load_date, record_source)
    (SELECT billing_period_pk, billing_period_key, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_hub_billing_period_etl);
    """
)
...
ins_t_link_payment = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_t_link_payment (PAY_PK, USER_PK, ACCOUNT_PK, BILLING_PERIOD_PK, sum, pay_doc_type, pay_doc_num, effective_from, load_date, record_source)
    (SELECT PAY_PK, USER_PK, ACCOUNT_PK, BILLING_PERIOD_PK, sum, pay_doc_type, pay_doc_num, effective_from, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_t_link_payment);
    """
)
...
ins_sat_user_details = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_sat_user_details (USER_PK, USER_HASHDIFF, phone, effective_from, load_date, record_source)
    (SELECT USER_PK, USER_HASHDIFF, phone, effective_from, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_sat_user_details_etl);
    """
)
...
ins_sat_payment_doc_type = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_sat_payment_doc_type (pk_id, pay_doc_type)
    (SELECT pk_id, pay_doc_type FROM rtk_de.ashanin_fprj_view_sat_payment_doc_type_etl);
    """
)
...
ins_sat_user_as_client = PostgresOperator(
    ...
    dag=dag,
    sql="""
        INSERT INTO rtk_de.ashanin_fprj_sat_user_as_client (USER_PK, legal_type, district, registered_at, billing_mode, is_vip, load_date, record_source)
    (SELECT USER_PK, legal_type, district, registered_at, billing_mode, is_vip, load_date, record_source
        FROM rtk_de.ashanin_fprj_view_sat_user_as_client_etl);
    """
)
...
create_temp_table = PostgresOperator(
    ...
    dag=dag,
    sql="""
    create table ashanin_fprj_payment_report_temp_{{ execution_date.year }} as (
with raw_data as (
  select 
    legal_type,
    district,
    ...
    from ashanin_fprj_t_link_payment tlp 
      join ashanin_fprj_hub_billing_period hbp
      on tlp.billing_period_pk = hbp.billing_period_pk
      
      join ashanin_fprj_hub_user hu 
      on hu.user_pk = tlp.user_pk
      
      left join ashanin_fprj_sat_user_as_client suac on suac.user_pk = hu.user_pk --- вместо mdm."user" - sat_user_as_client
      )
...
alter_temp_table = PostgresOperator(
    ...
    dag=dag,
    sql="""
    ALTER TABLE ashanin_fprj_payment_report_temp_{{ execution_date.year }} OWNER TO ashanin;
  """
)
...
ins_dim_billing_year = PostgresOperator(
    ...
    dag=dag,
    sql="""
  insert into ashanin_fprj_payment_report_dim_billing_year (billing_year_key)
  with distinct_temp as (
    select distinct billing_year from ashanin_fprj_payment_report_temp_{{ execution_date.year }}
  ), distinct_tbl as (
    select distinct billing_year_key from ashanin_fprj_payment_report_dim_billing_year
  ), new_values as (
...
ins_dim_legal_type = PostgresOperator(
    ...
    dag=dag,
    sql="""
        insert into ashanin_fprj_payment_report_dim_legal_type (legal_type_key)
  with distinct_temp as (
    select distinct legal_type from ashanin_fprj_payment_report_temp_{{ execution_date.year }}
  ), distinct_tbl as (
    select distinct legal_type_key from ashanin_fprj_payment_report_dim_legal_type
  ), new_values as (
...
ins_dim_district = PostgresOperator(
    ...
    dag=dag,
    sql="""
  insert into ashanin_fprj_payment_report_dim_district (district_key)
  with distinct_temp as (
    select distinct district from ashanin_fprj_payment_report_temp_{{ execution_date.year }}
  ), distinct_tbl as (
    select distinct district_key from ashanin_fprj_payment_report_dim_district
  ), new_values as (
...
ins_dim_registration_year = PostgresOperator(
    ...
    dag=dag,
    sql="""
      insert into ashanin_fprj_payment_report_dim_registration_year (registration_year_key)
  with distinct_temp as (
    select distinct registration_year from ashanin_fprj_payment_report_temp_{{ execution_date.year }}
  ), distinct_tbl as (
    select distinct registration_year_key from ashanin_fprj_payment_report_dim_registration_year
  ), new_values as (
...
ins_report_fact = PostgresOperator(
    ...
    dag=dag,
    sql="""
        insert into ashanin_fprj_payment_report_fact(billing_year_id, legal_type_id, district_id, registration_year_id, is_vip ,sum)
          select dby.id , dlt.id, dd.id, dry.id, raw.is_vip, raw.sum
            from ashanin_fprj_payment_report_temp_{{ execution_date.year }} raw 
              join ashanin_fprj_payment_report_dim_billing_year dby on dby.billing_year_key = raw.billing_year
              join ashanin_fprj_payment_report_dim_district dd on dd.district_key = raw.district
              join ashanin_fprj_payment_report_dim_legal_type dlt on dlt.legal_type_key = raw.legal_type
              join ashanin_fprj_payment_report_dim_registration_year dry on dry.registration_year_key = raw.registration_year;
        """
)
...
drop_temp_table = PostgresOperator(
    ...
    dag=dag,
    sql="""
    drop table if exists ashanin_fprj_payment_report_temp_{{ execution_date.year }};
    """
)
