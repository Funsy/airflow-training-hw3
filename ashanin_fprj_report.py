...
SQL_CONTEXT = {
    ...
    'DIMENSIONS': {
            'DIM_BILLING_YEAR':  """      
                    insert into ashanin.fprj_payment_report_dim_billing_year(billing_year_key)
                    select distinct billing_year as billing_year_key 
                    from ashanin.fprj_payment_report_tmp_{{ execution_date.year }} a
                    left join ashanin.fprj_payment_report_dim_billing_year b on b.billing_year_key = a.billing_year
                    where b.billing_year_key is null;
            """,
            'DIM_LEGAL_TYPE':  """
                    insert into ashanin.fprj_payment_report_dim_legal_type(legal_type_key)
                    select distinct legal_type as legal_type_key 
                    from ashanin.fprj_payment_report_tmp_{{ execution_date.year }} a
                    left join ashanin.fprj_payment_report_dim_legal_type b on b.legal_type_key = a.legal_type
                    where b.legal_type_key is null;
            """,
            'DIM_DISTRICT':  """
                    insert into ashanin.fprj_payment_report_dim_district(district_key)
                    select distinct district as district_key 
                    from ashanin.fprj_payment_report_tmp_{{ execution_date.year }} a
                    left join ashanin.fprj_payment_report_dim_district b on b.district_key = a.district
    ...
    'FACTS': {
            'REPORT_FACT':  """
                    insert into ashanin.fprj_payment_report_fct(
                                billing_year_id,
                                legal_type_id,
                                district_id,
                    ...
                    from ashanin.fprj_payment_report_tmp_{{ execution_date.year }} raw
